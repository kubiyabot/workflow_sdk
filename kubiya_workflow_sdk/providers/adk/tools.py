...
class KubiyaContextTools:
    ...
    def __post_init__(self):
        ...
        if self.api_key:
            self.client = KubiyaClient(
                api_key=self.api_key, base_url=self.base_url, org_name=self.org_name
            )
    ...
    def get_runners(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "runners": runners}

        except Exception as e:
            logger.error(f"Error fetching runners: {str(e)}")
            return {"status": "error", "message": str(e), "runners": []}
    ...
    def get_secrets_metadata(self) -> Dict[str, Any]:
        ...
        try:
            if not self.client:
                return {"status": "error", "message": "No API key provided", "secrets": []}
            ...
            return {"status": "error", "message": str(e), "secrets": []}
        ...
        try:
            if not self.client:
                return {
                    "status": "error",
                    "message": "No API key provided",
                    "organization": {"id": "unknown", "name": "Unknown Organization"},
                }

            # Use the SDK's get_organization_info method
            org_info = self.client.get_organization_info()

            return {"status": "success", "organization": org_info}

        except Exception as e:
            logger.error(f"Error fetching organization info: {str(e)}")
            return {"status": "error", "message": str(e), "organization": {}}
    ...
    def list_agents(self, search: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            if not self.client:
                return {"status": "error", "message": "No API key provided", "agents": []}
    ...
    def create_kubiya_agent(
        ...
        try:
            if not self.client:
                return {"status": "error", "message": "No API key provided", "agent": None}
            ...
            return {"status": "error", "message": str(e), "agent": None}
    ...
    def check_missing_context(
        ...
        """
        missing = {"secrets": [], "integrations": [], "runners": [], "other": []}
        ...
        try:
            ...
            return {"status": "success", "has_missing": any(missing.values()), "missing": missing}

        except Exception as e:
            logger.error(f"Error checking missing context: {str(e)}")
            return {"status": "error", "message": str(e), "has_missing": False, "missing": missing}
