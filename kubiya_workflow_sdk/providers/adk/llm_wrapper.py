...
class TogetherAILLM(BaseLlm):
            ...
            config["messages"].insert(
                0,
                {
                    ...
                    "content": (
                        ...
                        if hasattr(request.system_instruction, "parts")
                        else str(request.system_instruction)
                    ),
                },
            )
    ...
    async def generate_content_async(self, request: Any, **kwargs) -> AsyncIterator[Any]:
        ...
        try:
            ...
            response = await acompletion(**litellm_request, stream=True, api_key=self.api_key)
