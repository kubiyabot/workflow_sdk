...
class SSEFormatter:
    ...
    def format_tool_call(self, tool_name: str, args: Dict[str, Any]) -> str:
        ...
        data = {"type": "tool_call", "tool": tool_name, "args": args}
        return f"data: {json.dumps(data)}\n\n"

    def format_tool_result(self, tool_name: str, result: Any) -> str:
        """Format tool result as SSE."""
        data = {"type": "tool_result", "tool": tool_name, "result": result}
        return f"data: {json.dumps(data)}\n\n"
...
class VercelAIFormatter:
    ...
    def format_text(self, text: str) -> str:
        ...
        data = {"type": "text", "id": event_id, "content": text}
        return f"data: {json.dumps(data)}\n\n"
    ...
    def format_error(self, error: str) -> str:
        ...
        data = {"type": "error", "error": {"message": error, "type": "provider_error"}}
        return f"data: {json.dumps(data)}\n\n"
    ...
    def format_metadata(self, metadata: Dict[str, Any]) -> str:
        ...
        data = {"type": "metadata", "metadata": metadata}
        return f"data: {json.dumps(data)}\n\n"
...
class StreamHandler:
    ...
    def handle_event(self, event: Dict[str, Any]) -> None:
        ...
        if event_type == "text":
            ...
            formatted = self.formatter.format_tool_call(event["tool"], event.get("args", {}))
            self.events.append(formatted)

        elif event_type == "tool_result":
            formatted = self.formatter.format_tool_result(event["tool"], event.get("result"))
            self.events.append(formatted)
    ...
    async def async_handle_event(self, event: Dict[str, Any]) -> Optional[str]:
        ...
        if event_type == "text":
            ...
            return self.formatter.format_tool_call(event["tool"], event.get("args", {}))
        elif event_type == "tool_result":
            return self.formatter.format_tool_result(event["tool"], event.get("result"))
        elif event_type == "error":
