...
async def example_basic_generation():
    ...
    workflow = provider.generate_workflow(
        task=task, context={"output_format": "json", "alert_threshold": 80}
    )
...
async def example_streaming_generation():
    ...
    async for event in provider.generate_workflow(
        ...
        stream=True,
        stream_format="sse",
        context={"app_name": "my-node-app", "namespace": "production", "replicas": 3},
    ):
            ...
            try:
                ...
                if event.startswith("data: "):
                    ...
                    if "content" in data and "<workflow>" in data["content"]:
                        ...
                        match = re.search(
                            r"<workflow>\s*(.*?)\s*</workflow>", data["content"], re.DOTALL
                        )
                        if match:
...
async def example_with_platform_context():
    ...
    if not os.getenv("KUBIYA_API_KEY") or os.getenv("KUBIYA_API_KEY") == "mock":
        ...
        client.get_secrets_metadata = Mock(
            return_value=[
    ...
    workflow = provider.generate_workflow(task=task, session_id=session_id, user_id="demo-user")
...
"""
    ...
    if not validation["valid"]:
        ...
        refined_workflow = provider.refine_workflow(
            workflow_code=workflow_code, errors=validation["errors"]
        )
...
async def example_vercel_streaming():
    ...
    async for event in provider.generate_workflow(
        ...
        context={"deployment_type": "blue-green", "traffic_steps": [25, 50, 75, 100]},
    ):
