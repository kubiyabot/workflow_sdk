...
class LoopOrchestratorAgent(BaseAgent):
    ...
    def __init__(
        ...
        self._refinement_loop = RefinementLoop(self._refiner, self._compiler, max_iterations=3)
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncIterator[Event]:
        ...
        user_requirements = (
            ctx.user_content.parts[0].text if ctx.user_content and ctx.user_content.parts else ""
        )
        ...
        yield Event(
            ...
            content=types.Content(role="assistant", parts=[types.Part(text=start_message)]),
            partial=False,
        )
        ...
        while overall_iterations < self._max_iterations and not success:
            ...
            try:
                ...
                platform_context["available_runners"] = (
                    [r.get("name", r.get("id", "unknown")) for r in runners]
                ...
                platform_context["available_secrets"] = (
                    [s.get("name", s.get("key", "unknown")) for s in secrets]
                ...
                if event.content and event.content.parts:
                    ...
                    if event.is_final_response():
                        ...
                        code_match = re.search(
                            r"<workflow_code>\s*(.*?)\s*</workflow_code>", final_response, re.DOTALL
                        )
                        if code_match:
            ...
            if not compilation_result or compilation_result.get("status") != "success":
                ...
                errors = (
                    compilation_result.get("errors", ["Unknown compilation error"])
                ...
                for error in errors:
                    yield Event(
                        ...
                        invocation_id=invocation_id,
                        content=types.Content(
                            role="assistant", parts=[types.Part(text=f"- {error}\n")]
                        ),
                        partial=True,
                    )
                ...
                refinement_result = await self._refinement_loop.refine_until_success(
                    workflow_code, errors, platform_context, session
                )
                ...
                if refinement_result["success"]:
                    ...
                    yield Event(
                        ...
                        content=types.Content(
                            ...
                            parts=[
                                types.Part(
                                    text=f"❌ Refinement failed: {refinement_result['reason']}. Retrying from scratch..."
                                )
                            ],
                        ),
                        partial=False,
            ...
            if self._config.execute_workflows or compose_mode == "act":
                ...
                exec_params = (
                    compose_parameters if compose_parameters else final_workflow.get("params", {})
                )
                                ...
                                if result_match:
                                    try:
                                        ...
                                        execution_result = type(
                                            "ExecutionResult", (), exec_result_data
                                        )()
            ...
            if validation_result and validation_result.get("meets_requirements"):
                ...
                if hasattr(session, "context") and hasattr(session.context, "save_artifact"):
                    ...
                    version = await session.context.save_artifact(
                        filename="user:final_workflow.json", artifact=workflow_artifact
                    )

                    # Also save with timestamp
                    timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
                    await session.context.save_artifact(
                        filename=f"user:workflow_{timestamp}.json", artifact=workflow_artifact
                    )
                ...
                session.state["workflow_requirements"] = (
...
def create_loop_orchestrator_agent(
    ...
    return LoopOrchestratorAgent(
        config, context_tools, kubiya_client, artifact_service, max_iterations
    )
