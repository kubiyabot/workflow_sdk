    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncIterator[Event]:
        ...
        yield Event(
            ...
            content=types.Content(
                ...
                parts=[
                    types.Part(
                        text=f"🚀 Starting execution of workflow '{workflow_json.get('name')}'...\n"
                    )
                ],
            ),
            partial=False,
        ...
        org_name = (
            self._kubiya_client.org_name if hasattr(self._kubiya_client, "org_name") else "unknown"
        )
        ...
        try:
            ...
            yield Event(
                ...
                content=types.Content(
                    ...
                    parts=[
                        types.Part(
                            text=f"📡 Starting SSE stream for execution ID: {execution_id}\n\n"
                        )
                    ],
                ),
                partial=True,
            ...
            try:
                for event_data in self._kubiya_client.execute_workflow(
                    workflow_definition=workflow_json, parameters=params, stream=True
                ):
                    event_count += 1
                    ...
                    try:
                        ...
                        if isinstance(event, dict):
                            ...
                            if event_type == "start":
                                ...
                                if step_output:
                                    output_str += (
                                        f" with output: {json.dumps(step_output, indent=2)}"
                                    )
                                yield Event(
                                    author=self.name,
                                    invocation_id=invocation_id,
                                    content=types.Content(
                                        role="assistant", parts=[types.Part(text=output_str + "\n")]
                                    ),
                                    partial=True,
                                )
                            elif event_type == "log":
                                ...
                                yield Event(
                                    ...
                                    invocation_id=invocation_id,
                                    content=types.Content(
                                        role="assistant", parts=[types.Part(text=f"📝 {log_msg}\n")]
                                    ),
                                    partial=True,
                                )
                                ...
                                yield Event(
                                    ...
                                    content=types.Content(
                                        role="assistant",
                                        parts=[
                                            types.Part(text="✅ Workflow execution completed\n")
                                        ],
                                    ),
                                    partial=True,
                                )
                                ...
                                yield Event(
                                    ...
                                    content=types.Content(
                                        role="assistant",
                                        parts=[
                                            types.Part(text=f"{event_type}: {json.dumps(event)}\n")
                                        ],
                                    ),
                                    partial=True,
                                )
            ...
            yield Event(
                author=self.name,
                invocation_id=invocation_id,
                content=types.Content(role="assistant", parts=[types.Part(text=final_response)]),
                partial=False,
                turn_complete=True,
            )
            ...
            yield Event(
                ...
                invocation_id=invocation_id,
                content=types.Content(
                    role="assistant", parts=[types.Part(text=f"❌ Execution error: {str(e)}")]
                ),
                partial=False,
                turn_complete=True,
            )
...
"""
    ...
    def _get_execution_logs_tool(
        self, execution_id: str, step_name: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if hasattr(self, "_execution_events"):
            ...
            return {"success": True, "execution_id": execution_id, "logs": logs}
    ...
    def _cancel_execution_tool(self, execution_id: str) -> Dict[str, Any]:
        ...
        return {"success": False, "error": "Cancel endpoint not available"}

    def _before_tool_callback(
        self, tool_name: str, args: Dict[str, Any], callback_context: CallbackContext
    ...
    async def _after_model_callback(
        self, llm_response: Any, callback_context: CallbackContext
    ) -> Optional[Any]:
    ...
    async def execute_workflow(
        ...
        try:
            ...
            org_name = (
                self._kubiya_client.org_name
                if hasattr(self._kubiya_client, "org_name")
                else "unknown"
            )
            ...
            logger.info(
                f"   API Endpoint: {self._kubiya_client.base_url if hasattr(self._kubiya_client, 'base_url') else 'default'}"
            )
            ...
            try:
                ...
                for event_data in self._kubiya_client.execute_workflow(
                    ...
                    logger.info(
                        f"   Raw: {event_data[:200]}..."
                        if len(event_data) > 200
                        else f"   Raw: {event_data}"
                    )
                    ...
                    try:
                        ...
                        if isinstance(event, dict):
                            ...
                            if event_type == "start":
                                ...
                                if step_output:
                                    logger.info(
                                        f"      Output: {json.dumps(step_output, indent=2)}"
                                    )
                            elif event_type == "log":
                                log_msg = event.get("message", "")
                                logs.append(log_msg)
            ...
            traceback.print_exc()
            return WorkflowExecutionResult(success=False, errors=[str(e)])


def create_workflow_executor_agent(config: ADKConfig, kubiya_client: Any) -> WorkflowExecutorAgent:
