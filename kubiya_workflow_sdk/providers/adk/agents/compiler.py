    ...
    async def _after_model_callback(
        ...
        try:
            ...
            result_match = re.search(
                r"<compilation_result>\s*(.*?)\s*</compilation_result>", response_text, re.DOTALL
            )
            ...
            if result_match and hasattr(callback_context, "save_artifact"):
                try:
                    ...
                    version = await callback_context.save_artifact(
                        filename="compilation_result.json", artifact=result_artifact
                    )
                    ...
                    if result_json.get("status") == "success" and "workflow" in result_json:
                        ...
                        version = await callback_context.save_artifact(
                            filename="compiled_workflow.json", artifact=workflow_artifact
                        )
                        ...
                        if hasattr(callback_context, "session"):
                            callback_context.session.state["compiled_workflow"] = result_json[
                                "workflow"
                            ]
                        ...
                        if hasattr(callback_context, "session"):
                            callback_context.session.state["compilation_errors"] = result_json.get(
                                "errors", []
                            )
            ...
            if executor.get("type") == "tool":
                ...
                if tool_def:
                    if not tool_def.get("image"):
                        errors.append(
                            f"Step '{step.get('name', i+1)}' tool definition must have an image"
                        )
                    if not tool_def.get("content"):
                        warnings.append(
                            f"Step '{step.get('name', i+1)}' tool definition has no content"
                        )
