...
class ADKProvider(BaseProvider):
    ...
    async def _generate_workflow_sync(
        ...
        logger.info(
            f"Creating new session: app={self.runner.app_name}, user={user_id}, session={session_id}"
        )
        ...
        async for event in self.runner.run_async(
            ...
            event_count += 1
            logger.debug(
                f"Received event {event_count}: {type(event)}, turn_complete={getattr(event, 'turn_complete', False)}"
            )
            ...
            if event.content and event.content.parts and event.content.parts[0].text:
                ...
                workflow_match = re.search(
                    r"<workflow>\s*(.*?)\s*</workflow>", response_text, re.DOTALL
                )
                if workflow_match:
                    try:
    ...
    async def _stream_workflow_generation(
        ...
        formatter = VercelAIFormatter() if stream_format == "vercel" else SSEFormatter()
    ...
    def _format_adk_event(
        self, event: Event, formatter: Union[SSEFormatter, VercelAIFormatter]
    ) -> Optional[str]:
        ...
        if event.actions and event.actions.artifact_delta:
            if hasattr(formatter, "format_metadata"):
                return formatter.format_metadata({"artifacts_saved": event.actions.artifact_delta})
        ...
        **kwargs,
        ...
        try:
            ...
            workflow_artifact = await self._save_workflow_artifact(
                workflow=workflow, user_id=user_id, session_id=session_id
            )
            ...
            code_artifact = types.Part.from_bytes(
                data=execution_code.encode(), mime_type="text/x-python"
            )

            # Get session context
            try:
                session = await self.session_service.get_session(
                    app_name="kubiya_workflow_generator", user_id=user_id, session_id=session_id
                )
            except Exception:
            ...
            exec_version = session.context.save_artifact(
                filename="execution_code.py", artifact=code_artifact
            )
            ...
            if stream:
                ...
                result_artifact = types.Part.from_bytes(
                    data=json.dumps(result, indent=2).encode(), mime_type="application/json"
                )
                result_version = session.context.save_artifact(
                    filename="execution_result.json", artifact=result_artifact
                )
    ...
    async def _save_workflow_artifact(
        self, workflow: Union[str, Dict[str, Any]], user_id: str, session_id: str
    ) -> int:
        ...
        if isinstance(workflow, dict):
            ...
            artifact = types.Part.from_bytes(
                data=json.dumps(workflow, indent=2).encode(), mime_type="application/json"
            )
        ...
        version = session.context.save_artifact(filename=filename, artifact=artifact)

        logger.info(f"Saved workflow as artifact '{filename}' version {version}")
        return version

    def _generate_execution_code(
        self, workflow: Union[str, Dict[str, Any]], parameters: Optional[Dict[str, Any]] = None
    ) -> str:
...
"""
    ...
    async def _stream_sdk_execution(
        self, execution_code: str, stream_format: str, user_id: str, session_id: str, **kwargs
    ) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            exec_message = types.Content(
                ...
                parts=[
                    types.Part(
                        text=f"Execute this workflow code and stream the results:\n\n```python\n{execution_code}\n```"
                    )
                ],
            )
            ...
            async for event in self.runner.run_async(
                user_id=user_id, session_id=session_id, new_message=exec_message
            ):
    ...
    async def _execute_sdk_workflow(self, execution_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            namespace = {"KubiyaClient": self.client.__class__, "client": self.client}
    ...
    def validate_workflow(
        ...
        try:
            ...
            if not workflow_dict:
                return {"valid": False, "errors": ["No workflow found"], "warnings": []}
...
"""
    ...
    def _validate_with_sdk(self, workflow_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"valid": False, "errors": [f"SDK validation error: {str(e)}"], "warnings": []}
    ...
    def compose(
        ...
        **kwargs,
        ...
        compose_message = types.Content(role="user", parts=[types.Part(text=task)])
        ...
        async for event in self.runner.run_async(
            ...
            if event.content and event.content.parts and event.content.parts[0].text:
                ...
                exec_match = re.search(
                    r"<execution_result>\s*(.*?)\s*</execution_result>", response_text, re.DOTALL
                )
                if exec_match:
                    try:
        ...
        try:
            ...
            if self.config.model_provider == ModelProvider.TOGETHER_AI:
                ...
                if api_key:
                    ...
                    logger.warning(
                        "⚠️  TOGETHER_API_KEY not found - Together AI models may not work"
                    )
                ...
                if os.getenv("GOOGLE_APPLICATION_CREDENTIALS"):
                    ...
                    logger.warning(
                        "⚠️  GOOGLE_APPLICATION_CREDENTIALS not found - Vertex AI models may not work"
                    )
            ...
            logger.debug(
                f"Models configured: {', '.join([self.config.get_model_for_role(role) for role in ['orchestrator', 'context_loader', 'workflow_generator', 'compiler', 'refinement', 'fast']])}"
            )
