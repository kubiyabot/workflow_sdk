...
class OrchestrationServer(WorkflowServer):
    ...
    def _init_providers(self):
        ...
        if ProviderMode.ADK not in self.providers:
            try:
                ...
                # Create ADK provider
                self.providers[ProviderMode.ADK] = ADKProvider(client=client, config=config)
    ...
    def _register_orchestration_routes(self):
        ...
        async def compose_workflow(request: ComposeRequest):
            ...
            try:
                ...
                if not provider:
                    raise HTTPException(
                        status_code=400, detail=f"Provider mode '{request.mode}' not available"
                    )
        ...
        async def list_providers():
            ...
            return {"providers": providers_info, "default": self.default_mode}

    async def _stream_composition(
        self, provider: Any, request: ComposeRequest
        ...
        try:
            ...
            if hasattr(provider, "compose"):
                ...
                if request.stream_format == "vercel":
                    error_event = {
                        "id": str(uuid.uuid4()),
                        "type": "error",
                        "error": {"message": error_msg, "code": "COMPOSE_NOT_SUPPORTED"},
                    }
                else:
                    error_event = {
                        "event": "error",
            ...
            if request.stream_format == "vercel":
                error_event = {
                    "id": str(uuid.uuid4()),
                    "type": "error",
                    "error": {"message": str(e), "code": "COMPOSITION_ERROR"},
                }
            else:
                error_event = {
                    "event": "error",
    ...
    async def _stream_generation(
        self, provider: Any, request: GenerateRequest
    ) -> AsyncGenerator[str, None]:
    ...
    async def _stream_execution(
        self, provider: Any, request: ExecuteRequest
    ) -> AsyncGenerator[str, None]:
    ...
    async def _stream_refinement(
        self, provider: Any, request: RefineRequest
    ) -> AsyncGenerator[str, None]:
    ...
    async def _compose_workflow(self, provider: Any, request: ComposeRequest) -> Dict[str, Any]:
