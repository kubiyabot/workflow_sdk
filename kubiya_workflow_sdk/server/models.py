...
class BaseRequest(BaseModel):
    ...
    model_config = ConfigDict(
        extra="forbid", str_strip_whitespace=True, json_schema_extra={"example": {}}
    )
...
class WorkflowExecutionRequest(BaseRequest):
    ...
    workflow: Dict[str, Any] = Field(
        ...
        json_schema_extra={
            "example": {"name": "example-workflow", "version": "1.0.0", "steps": []}
        },
    )
    parameters: Optional[Dict[str, Any]] = Field(
        default=None, description="Runtime parameters for workflow execution"
    )
    options: Optional[Dict[str, Any]] = Field(
        default=None, description="Execution options (timeout, retry policy, etc.)"
    )
...
class WorkflowExecutionResponse(BaseResponse):
    ...
    stream_url: Optional[str] = Field(None, description="URL for SSE streaming of execution events")
...
class WorkflowStatusResponse(BaseResponse):
    ...
    progress_percentage: float = Field(
        default=0.0, ge=0, le=100, description="Execution progress percentage"
    )
...
class WorkflowValidationRequest(BaseRequest):
    ...
    strict: bool = Field(default=True, description="Enable strict validation mode")
    validate_executors: bool = Field(default=True, description="Validate executor configurations")
...
class PythonExecutionRequest(BaseRequest):
    ...
    code: str = Field(
        ..., description="Python code to execute", min_length=1, max_length=1_000_000  # 1MB limit
    )
    context: Optional[Dict[str, Any]] = Field(
        default=None, description="Execution context (variables, imports, etc.)"
    )
...
class PythonExecutionResponse(BaseResponse):
    ...
    results: Dict[str, Any] = Field(
        default_factory=dict, description="Execution results (variables)"
    )
...
class ServerHealthResponse(BaseResponse):
    ...
    max_executions: int = Field(..., description="Maximum concurrent executions allowed")
...
class ServerConfigResponse(BaseResponse):
    ...
    features: Dict[str, bool] = Field(..., description="Enabled server features")
    supported_executors: List[ExecutorType] = Field(default_factory=lambda: list(ExecutorType))
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "details": {"field": "steps", "message": "Steps cannot be empty"},
...
class BatchWorkflowExecutionRequest(BaseRequest):
    ...
    workflows: List[WorkflowExecutionRequest] = Field(
        ..., min_length=1, max_length=100, description="List of workflows to execute"
    )
    ...
    stop_on_failure: bool = Field(
        default=False, description="Stop batch execution on first failure"
    )
...
class BatchWorkflowExecutionResponse(BaseResponse):
    ...
    total_workflows: int
    execution_ids: List[str] = Field(..., description="Individual workflow execution IDs")
