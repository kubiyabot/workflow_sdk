...
class WorkflowServer:
    ...
    def __init__(
        ...
        self.execution_manager = ExecutionManager(
            max_concurrent=max_concurrent_executions, default_timeout=execution_timeout
        )
    ...
    def _register_routes(self, app: FastAPI) -> None:
                ...
                if stream:
                    ...
                    return EventSourceResponse(
                        self._stream_workflow_execution(
                            execution_id, request.workflow, request.parameters, api_token
                        ),
                        headers={
        ...
        @app.post(
            f"{self.api_prefix}/workflows/validate", response_model=WorkflowValidationResponse
        )
        async def validate_workflow(request: WorkflowValidationRequest):
            ...
            try:
                ...
                return WorkflowValidationResponse(
                    valid=False, errors=[str(e)], warnings=[], info=[]
                )
        ...
        async def execute_python(request: PythonExecutionRequest, stream: bool = True):
            ...
            try:
                ...
                if stream:
                    return EventSourceResponse(
                        self._stream_python_execution(execution_id, request.code, request.context),
                        headers={
                            "Cache-Control": "no-cache",
                            "X-Accel-Buffering": "no",
                            "X-Execution-ID": execution_id,
                    ...
                    result = await self._execute_python_code(request.code, request.context)
        ...
        async def list_executions(
            status: Optional[WorkflowStatus] = None, limit: int = 100, offset: int = 0
        ):
    ...
    async def _stream_workflow_execution(
        ...
        try:
            ...
            async for event in client.execute_workflow_stream(
                workflow=workflow_dict, params=parameters
            ):
    ...
    async def _stream_python_execution(
        self, execution_id: str, code: str, context: Optional[Dict[str, Any]]
    ) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    async def _execute_workflow_background(
        ...
        try:
            client = StreamingKubiyaClient(api_token=api_token)
            result = await client.execute_workflow_async(workflow=workflow_dict, params=parameters)
    ...
    async def _replay_execution_events(
        ...
        for event in events:
            yield {"event": event.get("type", "replay"), "data": json.dumps(event)}
        ...
        while True:
            ...
            yield {
                "event": "keep-alive",
                "data": json.dumps(
                    {"execution_id": execution_id, "timestamp": datetime.utcnow().isoformat()}
                ),
            }
...
__all__ = ["WorkflowServer", "create_server"]
