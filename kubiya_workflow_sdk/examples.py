...
def build_parallel_processing_workflow() -> Workflow:
    """Build a workflow demonstrating parallel processing capabilities"""

    # Parallel processing step
    parallel_process_step = ParallelStep(
        name='parallel_data_processing',
        description='Process multiple data files in parallel',
        parallel=ParallelConfig(
            items=['file1.csv', 'file2.csv', 'file3.csv', 'file4.csv'],
            max_concurrent=2,
        ),
        command='python process_file.py --input=${ITEM} --output=processed_${ITEM}',
        output='PROCESSED_FILES',
    )

    # Aggregation step
    aggregate_step = CommandStep(
        name='aggregate_results',
        description='Aggregate all processed files',
        depends=[parallel_process_step.name],
        command='python aggregate.py --input_pattern="processed_*.csv" --output=final_result.json',
        output='AGGREGATED_RESULT',
    )

    return Workflow(
        name='parallel_processing_demo',
        description='Demonstrate parallel processing with aggregation',
        steps=[
            parallel_process_step,
            aggregate_step,
        ],
        tags=['parallel', 'processing', 'demo'],
    )


def build_conditional_workflow() -> Workflow:
    """Build a workflow demonstrating conditional execution"""

    # Environment check step
    env_check_step = CommandStep(
        name='environment_check',
        description='Check if we are in production environment',
        command='echo $ENVIRONMENT',
        output='CURRENT_ENV',
    )

    # Production deployment step (conditional)
    prod_deploy_step = CommandStep(
        name='production_deployment',
        description='Deploy to production environment',
        depends=[env_check_step.name],
        command='./deploy_production.sh',
        preconditions=[
            Precondition(
                condition='${CURRENT_ENV}',
                expected='production',
    ...
    # Development deployment step (conditional)
    dev_deploy_step = CommandStep(
    ...
    notify_step = CommandStep(
        name='send_notification',
        description='Send deployment notification',
        depends=[prod_deploy_step.name, dev_deploy_step.name],
        command='echo "Deployment completed for environment: ${CURRENT_ENV}"',
    )
    ...
    return Workflow(
        ...
        tags=['deployment', 'conditional', 'environment'],
    )
...
if __name__ == '__main__':
    ...
    print("Main Workflow:")
    print(main_workflow.model_dump_json(indent=2))

    # Build additional example workflows
    parallel_workflow = build_parallel_processing_workflow()
    conditional_workflow = build_conditional_workflow()
