...
class FastMCP:
    ...
    def __init__(
        ...
        self.kubiya_client = (
            KubiyaClient(api_key=api_token, base_url=base_url) if api_token else None
        )
        ...
        *,
        ...
        def decorator(f: Callable) -> Callable:
            ...
            self.tools[tool_name] = Tool(
                name=tool_name, description=tool_desc, parameters=params, handler=f
            )
    ...
    def _register_builtin_tools(self):
        ...
        async def execute_workflow(
            name: str, params: Optional[Dict[str, Any]] = None, stream: bool = True
        ) -> Dict[str, Any]:
            """
            ...
            try:
                ...
                if stream:
                    ...
                    asyncio.create_task(self._stream_execution(execution_id, workflow, params))
                    ...
                    result = self.kubiya_client.execute_workflow(
                        workflow_definition=workflow, params=params
                    )
        ...
        async def validate_workflow(workflow_def: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
            ...
            try:
                ...
                return {"success": False, "valid": False, "errors": [str(e)]}
        ...
        if HAS_GRAPHQL:
            ...
            async def graphql_query(
                ...
                try:
                    ...
                    if result.errors:
                        return {"success": False, "errors": [str(e) for e in result.errors]}
    ...
    async def _stream_execution(
        self, execution_id: str, workflow: Dict[str, Any], params: Optional[Dict[str, Any]]
    ):
        ...
        try:
            ...
            def sync_events():
                for event in execute_workflow_events(
                    workflow_definition=workflow, api_key=self.api_token, parameters=params
                ):
                    yield event
            ...
            for event in sync_events():
                ...
                if "step" in event:
                    ...
                    self.executions[execution_id]["errors"] = self.executions[execution_id].get(
                        "errors", []
                    )
            ...
            def resolve_steps(self, info):
                ...
                return [
                    StepType(
                        ...
                        depends_on=(
                            step.get("depends", [])
                            if isinstance(step.get("depends"), list)
                            else [step.get("depends")] if step.get("depends") else []
                        ),
                    )
        ...
        while True:
            try:
                ...
                if command.get("type") == "tool":
                    ...
                    print(json.dumps({"tools": self.list_tools()}))
                else:
                    print(json.dumps({"error": f'Unknown command type: {command.get("type")}'}))
...
class KubiyaMCP(FastMCP):
    ...
    def _register_kubiya_tools(self):
        ...
        async def validate_workflow_code(code: str) -> Dict[str, Any]:
            ...
            try:
                # Create a safe execution environment
                exec_globals = {"workflow": flow_decorator, "step": step, "__name__": "__main__"}
                ...
                if not workflow_func:
                    return {"success": False, "error": "No workflow found in code"}
