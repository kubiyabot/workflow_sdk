...
class KubiyaWorkflowServer:
    ...
    def __init__(
        ...
        self.kubiya_client = (
            KubiyaClient(api_key=self.api_token, base_url=base_url) if self.api_token else None
        )
    ...
    def _register_tools(self):
        ...
        async def create_workflow_from_python(
            ...
            try:
                ...
                result = {
                    ...
                    "validation": {"valid": len(errors) == 0, "errors": errors, "warnings": []},
                    "workflow": {
        ...
        async def execute_workflow_streaming(
            ...
            execution_state = ExecutionState(
                execution_id=execution_id, status="starting", workflow_name=name
            )
        ...
        async def get_execution_status(
            execution_id: str, include_events: bool = False, ctx: Context = None
        ) -> Dict[str, Any]:
            """
            ...
            try:
                ...
                if source_type == "python":
                    ...
                    return await create_workflow_from_python(
                        name="__validation_temp__", code=workflow_data, validate_only=True, ctx=ctx
                    )
                else:
        ...
        async def get_workflow_definition(
            name: str, format: str = "json", ctx: Context = None
        ) -> Dict[str, Any]:
            """
    ...
    async def _execute_workflow_async(
        ...
        try:
            ...
            if ctx:
                await ctx.info(
                    f"Executing workflow with {len(workflow_def.get('steps', []))} steps"
                )

            # Execute using the SDK's execution module (convert sync to async)
            events_processed = 0
            for event in execute_workflow_events(
                workflow_definition=workflow_def, api_key=self.api_token, parameters=parameters
            ):
                # Store event
    ...
    def run(self, transport: str = "stdio", host: str = "127.0.0.1", port: int = 8000, **kwargs):
        """
...
def create_workflow_server(
    ...
    """
    return KubiyaWorkflowServer(name=name, api_token=api_token, base_url=base_url)
