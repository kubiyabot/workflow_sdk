...
<Tabs>
  ...
  <Tab title="Kubiya Approach">
    ...
    adk = get_provider("adk", client=client)
    workflow = adk.compose("Deploy my application", stream=False)
...
<Tabs>
  ...
  <Tab title="Kubiya Incident Response">
    ...
    workflow = (
        ...
        .step("detect", callback=lambda step:
            step.inline_agent(
                ai_instructions="You are an incident response agent. Analyze the incident and determine the severity.",
        ...
        .step("respond", callback=lambda step:
            step.shell("""
        ...
        .step("notify", callback=lambda step:
            step.shell("send-slack-notification.sh")
        )
    )
...
adk = get_provider("adk", client=client)
workflow = adk.compose("""Create a workflow that:
...
production_result = client.execute_workflow(workflow)
```
