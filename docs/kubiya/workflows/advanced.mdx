...
# test_workflow.py
import pytest
from kubiya_workflow_sdk import Workflow, LocalRunner

@pytest.fixture
def local_runner():
    return LocalRunner(docker_socket="/var/run/docker.sock")

def test_data_pipeline(local_runner):
    workflow = Workflow(
        name="test-pipeline",
        steps=[
    ...
    result = local_runner.execute(workflow)
