  ...
  <Step title="Stream Real-time Updates">
    ...
    for event in client.execute_workflow(my_workflow, stream=True):
        if event.type == "step_started":
            print(f"▶️  Starting: {event.step_name}")
        elif event.type == "step_completed":
            print(f"✅ Completed: {event.step_name}")
            if event.output:
                print(f"   Output: {event.output}")
        elif event.type == "workflow_completed":
            print(f"🎉 Workflow completed with status: {event.status}")
    ```
  </Step>
</Steps>
...
for event in client.execute_workflow(deployment_workflow, parameters=params, stream=True):
    if event.type == "step_started":
        print(f"▶️  {event.step_name}: Starting...")
    elif event.type == "step_completed":
        print(f"✅ {event.step_name}: {event.status}")
...
notification_workflow = (
    ...
    .args(
        channel="#deployments",
        text="🚀 Deployment started for v${VERSION}"
    )
