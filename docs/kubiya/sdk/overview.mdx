...
Simple and advanced step definitions:
...
shell_step = (
    Step(name="process-data")
        .description("shell step to process data")
        .shell(
            """
                #!/bin/sh
                echo "Hello, World!"
        ...
        .args(some_arg="${some_arg}")
)


# Container step with health checks
container_step = (
        Step(name="server-healthcheck")
            .description("Make a server health check")
            .docker(
                image="alpine:latest",
                content=
                """
